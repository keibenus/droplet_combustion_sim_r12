...
def run_simulation_split():
    ...
    if step_count_total == 0: # 新規実行またはリスタート失敗時
        ...
        _, Y_eq_prev_step = interface.calculate_fuel_mass_fraction_surf(T_l[-1] if Nl > 0 else T_g[0], P, gas_props, liquid_props)
        current_t = 0.0
        dt = config.DT_INIT
    ...
    while current_t < config.T_END and status == "Running":
        ...
        dt = numerics.calculate_adaptive_dt(
             ...
             r_g_nodes, r_l_nodes, dt, nsp) # Pass current dt
        ...
        while iter_count < config.MAX_INTERFACE_ITER and not converged:
            ...
            if not is_gas_phase_only:
                ...
                _, Y_eq_final_iter = interface.calculate_fuel_mass_fraction_surf(T_s_final_iter, P_n, gas_props, liquid_props)
